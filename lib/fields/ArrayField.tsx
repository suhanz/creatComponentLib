...
export default defineComponent({
  ...
  setup(props) {
    ...
    const dealUiSchema = function (itemUiSchema: any, index: number) {
      if (Array.isArray(itemUiSchema)) {
        return itemUiSchema[index]
      } else {
        return itemUiSchema || {}
      }
    }
      ...
      if (isMultiType) {
        ...
        return items.map((s, index: number) => (
          <SchemaItem
            schema={s}
            key={index}
            uiSchema={dealUiSchema(uiSchema.items, index)}
